...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class PostProcessLayer : MonoBehaviour
    {
        ...
        void SetLegacyCameraFlags(PostProcessRenderContext context)
        {
            ...
            foreach (var bundle in m_Bundles)
            {
                if (bundle.Value.settings.IsEnabledAndSupported(context))
                    flags |= bundle.Value.renderer.GetCameraFlags(context);
            }

            // Special case for AA & lighting effects
            if (context.IsTemporalAntialiasingActive())
                flags |= temporalAntialiasing.GetCameraFlags(context);

            if (fog.IsEnabledAndSupported(context))
                flags |= fog.GetCameraFlags(context);

            if (debugLayer.debugOverlay != DebugOverlay.None)
                flags |= debugLayer.GetCameraFlags(context);

            if ((context.camera.depthTextureMode & DepthTextureMode.Depth)!=0)
        ...
        int RenderBuiltins(PostProcessRenderContext context, bool isFinalPass, int releaseTargetAfterUse = -1, int eye = -1)
        {
            ...
            {
                RenderEffect<AdvanceFog>(context, true);
                RenderEffect<GodRay>(context, true);
            }
            ...
            //if (context.camera.cameraType == CameraType.Game)
            if (GlowOutlineRenderer.GetElementCount()>0)
            {
                RenderEffect<GlowOutline>(context, true);
            }
