...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    internal sealed class ScalableAO : IAmbientOcclusionMethod
    {
        ...
        bool canSkip(PostProcessRenderContext context)
        {
            bool reset = false;
            ...
            {
                float deltacampos = Vector3.Distance(cam.transform.position, oldcampos);
                float deltacamangle = Quaternion.Angle(cam.transform.rotation, oldcamangle);

                bool skip = true;
                if (reset || !Application.isPlaying || firstFrame || Time.frameCount % 10 == 0 || deltacampos > 0.01f || deltacamangle > 0.01f)
                {
                    skip = false;
                }
                return skip;
        ...
        public DepthTextureMode GetCameraFlags(PostProcessRenderContext context)
        {
            if (canSkip(context))
        ...
        bool DoLazyInitialization(PostProcessRenderContext context)
        {
        ...
        private Quaternion oldcamangle = Quaternion.identity;

        void Render(PostProcessRenderContext context, CommandBuffer cmd, int occlusionSource)
        {
            bool reset=DoLazyInitialization(context);
            ...
            if (cam != null && cam.enabled)
            {
                ...
                if (reset || !Application.isPlaying || firstFrame || Time.frameCount % 10 == 0 || deltacampos > 0.01f || deltacamangle > 0.01f)
                {
                    ...
                    oldcamangle = cam.transform.rotation;
                    firstFrame = false;
                }
                if (skip)
                    return;
