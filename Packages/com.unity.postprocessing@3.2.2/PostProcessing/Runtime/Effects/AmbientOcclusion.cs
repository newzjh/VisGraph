...
namespace UnityEngine.Rendering.PostProcessing
{
    ...
    public sealed class AmbientOcclusion : PostProcessEffectSettings
    {
        ...
        {
            if (context.resources == null || context.resources.shaders == null)
                return false;
            ...
            else if (mode.value == AmbientOcclusionMode.MultiScaleVolumetricObscurance)
            {
                if (context != null)
                {
                    state &= context.resources.shaders.multiScaleAO
                        ...
                        && context.resources.computeShaders.multiScaleAODownsample1.HasKernel("MultiScaleVODownsample1")
                        && context.resources.computeShaders.multiScaleAODownsample2
                        && context.resources.computeShaders.multiScaleAODownsample2.HasKernel("MultiScaleVODownsample2")
                        && context.resources.computeShaders.multiScaleAORender
                        && context.resources.computeShaders.multiScaleAORender.HasKernel("MultiScaleVORender_interleaved")
                        && context.resources.computeShaders.multiScaleAOUpsample
                        && context.resources.computeShaders.multiScaleAOUpsample.HasKernel("MultiScaleVOUpSample");
                }
    ...
    {
        DepthTextureMode GetCameraFlags(PostProcessRenderContext context);
    ...
    internal sealed class AmbientOcclusionRenderer : PostProcessEffectRenderer<AmbientOcclusion>
    {
        ...
        {
            return Get().GetCameraFlags(context);
        }
